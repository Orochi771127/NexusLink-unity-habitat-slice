...
namespace NexusLink.Moonlake.HeroSlice
{
    ...
    /// it owns nothing but this SpriteRenderer's configuration.
    ///
    /// That state now belongs to this project's own gameplay systems. The web
    /// build is the reference implementation for how it should behave, not the
    ...
    public sealed class MoonlakeCompanionBillboard : MonoBehaviour
    {
        ...
        [Header("Character animation set (preferred)")]
        [Tooltip("Imported from the character's approved animations.json. When " +
                 "assigned this supersedes the legacy direction arrays below.")]
        public MoonlakeCharacterAnimationSet animationSet;

        [Tooltip("Animation id to play while standing still. Which pose is " +
                 "emotionally appropriate is a gameplay decision; this component " +
                 "only renders the id it is given.")]
        public string poseAnimationId = "idle_calm";

        [Tooltip("Set by whatever drives locomotion. Presentation input only.")]
        public bool isMoving;

        [Header("Legacy direction frames (fallback)")]
        ...
        /// <summary>
        /// The walk sheet matching the current facing, falling back through the
        /// front sheet to the resting pose. Characters are authored to different
        /// levels of completeness, so a missing direction must degrade rather
        /// than blank the companion.
        /// </summary>
        static string WalkIdFor(Facing facing)
        {
            switch (facing)
            {
                case Facing.Back: return "back_walk";
                case Facing.Left: return "left_walk";
                case Facing.Right: return "right_walk";
                default: return "front_walk";
            }
        }

        MoonlakeCharacterAnimationSet.Clip ResolveClip()
        {
            if (animationSet == null) return null;
            var resting = string.IsNullOrEmpty(poseAnimationId) ? "idle_calm" : poseAnimationId;
            return isMoving
                ? animationSet.FindFirst(WalkIdFor(_facing), "front_walk", resting, "idle_calm")
                : animationSet.FindFirst(resting, "idle_calm", "front_walk");
        }

        void ApplyAnimation()
        {
            var clip = ResolveClip();
            var frames = clip != null ? clip.frames : LegacySet();
            var fps = clip != null ? clip.fps : framesPerSecond;
            var loop = clip == null || clip.loop;
            ...
            // Restart cleanly whenever the clip changes, so a 6-frame pose never
            // inherits a frame index from an 8-frame walk.
            var clipId = clip != null ? clip.animationId : "legacy:" + _facing;
            if (!string.Equals(clipId, _activeClipId, StringComparison.Ordinal))
            {
            ...
            if (animate && Application.isPlaying && fps > 0f)
            {
            ...
            if (loop) _frame = ((_frame % frames.Length) + frames.Length) % frames.Length;
            else _frame = Mathf.Clamp(_frame, 0, frames.Length - 1);
